...
// --- Identifier ---
// !!! IMPORTANT: SET TO 'L' FOR LEFT EMITTER, 'R' FOR RIGHT EMITTER !!!
const char myIdentifier = 'R'; // OR 'L'
...
// --- Control & Timing Settings ---
unsigned long lastSampleTime = 0;
const unsigned long SAMPLE_INTERVAL = 200; // Read sensor data interval (also used for IMU print during WiFi connect)
unsigned long lastSendTime = 0;
const unsigned long SEND_INTERVAL = 500; // Send data to server interval
...
void loop() {
    ...
    // --- Phase 2: WiFi Connection Management ---
    if (WiFi.status() != WL_CONNECTED) {
        static unsigned long lastWifiCheckTime = 0; // Used for printing "."
        static unsigned long lastImuReadTimePhase2 = 0; // Used for IMU print during this phase

        // Print "." periodically to show activity
        if (currentMillis - lastWifiCheckTime > 1000) {
            Serial.print(".");
            lastWifiCheckTime = currentMillis;
        }

        // Read and print IMU data periodically during WiFi connection attempts
        if (currentMillis - lastImuReadTimePhase2 >= SAMPLE_INTERVAL) { // Use SAMPLE_INTERVAL
            lastImuReadTimePhase2 = currentMillis;
            readAndCorrectData(); // Read MPU6050 data
            Serial.print("\n[WiFi Connecting] IMU -> Acc(g): "); // Indicate context
            Serial.print("X="); Serial.print(AccX, 4);
            Serial.print(" Y="); Serial.print(AccY, 4);
            Serial.print(" Z="); Serial.print(AccZ, 4);
            Serial.print(" | Gyro(dps): ");
            Serial.print("X="); Serial.print(GyroX, 2);
            Serial.print(" Y="); Serial.print(GyroY, 2);
            Serial.print(" Z="); Serial.println(GyroZ, 2);
            Serial.print("Attempting to connect to WiFi"); // Re-print the "." prompt starter
        }

        // Ensure effect is off if WiFi drops or is not yet connected
        if (rainbowEffectActive) {
             Serial.println("\nWiFi not connected, stopping effect.");
             rainbowEffectActive = false; strip.clear(); strip.show();
        }
        if (serverConnected) { // If server was connected but WiFi dropped
            Serial.println("\nWiFi disconnected, server connection lost.");
            serverConnected = false; client.stop();
        }
        return; // Stay in Phase 2 until WiFi is connected
    }
    ...
    if (!serverConnected) {
        Serial.println("\nWiFi Connected! Attempting server connection...");
        connectToServer();
        // Don't automatically start rainbow effect here anymore; it's controlled by receiver
    }

    // --- Check for Feedback from Receiver ---
    if (serverConnected) { // Only handle feedback if connected to server
        handleReceiverFeedback(); // Check for '0' or '1'
    }
    ...
    if (currentMillis - lastSampleTime >= SAMPLE_INTERVAL) {
        lastSampleTime = currentMillis;
        readAndCorrectData(); // Call the function defined below
        // IMU Data printout for debugging (when fully operational)
        Serial.print("IMU Data -> Acc(g): "); Serial.print("X="); Serial.print(AccX, 4); Serial.print(" Y="); Serial.print(AccY, 4); Serial.print(" Z="); Serial.print(AccZ, 4);
        Serial.print(" | Gyro(dps): "); Serial.print("X="); Serial.print(GyroX, 2); Serial.print(" Y="); Serial.print(GyroY, 2); Serial.print(" Z="); Serial.println(GyroZ, 2);
        detectCurrentMotion(); // Call the function defined below
    }
    ...
    // --- Send Motion Data ---
    if (serverConnected && (currentMillis - lastSendTime >= SEND_INTERVAL)) { // Only send if connected to server
        lastSendTime = currentMillis;
        int currentState = determineOverallState(); // Call the function defined below
        sendDataStable(currentState); // Sends motion state (0, 1, 2)
    }
...
void detectCurrentMotion() {
  // Use absolute values of *corrected* data
  // NOTE: There was a 'abs(AccZ-1)' in your provided code, which is unusual after calibration.
  // Corrected AccZ should be near 0 when stable. If it's near 1, the offsetAccZ calculation might be off
  // or it wasn't subtracted. For now, using abs(AccZ) as it's more standard after offset application.
  // If AccZ is consistently around 1.0 after calibration when stable, review `finishCalibration`.
  float absAccZ_deviation = abs(AccZ);
  float absGyroX = abs(GyroX);
  float absGyroZ = abs(GyroZ);
